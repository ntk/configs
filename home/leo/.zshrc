...
if [[ "$TERM" == dumb ]] ; then
    PROMPT="${EXITCODE}%n@%m %40<...<%B%~%b%<< "'${VCS_INFO_message_0_}'"%# "
else
...
${BLUE}%n${NO_COLOUR}@%m %40<...<%B%~%b%<< "'${VCS_INFO_message_0_}'"%# "
        ...
        if (( EUID != 0 )); then
            PROMPT="${RED}${EXITCODE}${MAGENTA}%n${NO_COLOUR}@${CYAN}%m${YELLOW} %${NO_COLOR}40<...<%B%~%b%<< "'${VCS_INFO_message_0_}'"%# "
        else
            PROMPT="${BLUE}${EXITCODE}${RED}%n${NO_COLOUR}@${BLUE}%m${YELLOW} #${NO_COLOR}40<...<%B%~%b%<< "'${VCS_INFO_message_0_}'"%# "
        fi
    fi
fi
...
alias rst='reset'
alias top='htop'
urxvt_size() { urxvt -fn "xft:Bitstream Vera Sans Mono:pixelsize=$1" -fb "xft:Bitstream Vera Sans Mono:pixelsize=$1"(/) } 
alias urxvts="urxvt_size"
alias clean='rm -f *~; rm -f *.class'
alias jamendo='unzip *.zip; rm *.jpg *.zip *.txt'
...
demap() {
    echo -n 'Switching to german keyboard layout: '
    [[ -z "$DISPLAY" ]] && $SUDO loadkeys de-latin1-nodeadkeys &>/dev/null || setxkbmap de &>/dev/null
    echo 'Done'
}
